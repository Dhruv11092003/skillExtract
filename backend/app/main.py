...
def _persist_upload_bytes(pdf_bytes: bytes) -> Path:
    ...
    Using NamedTemporaryFile(delete=True) can fail on Windows because the open
    handle prevents a second open by pdfplumber. mkstemp closes safely and is
    cross-platform for downstream readers.
    """
...
async def analyze_resume(
    ...
    for skill in skills:
        ...
        _, page_height = parsed.page_sizes.get(token.coordinate.page, (1.0, 1.0))
